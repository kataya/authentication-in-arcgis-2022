...
- [esm-api-key-demo](./esm-api-key-demo/README.md) は ES モジュールを使用した  [ArcGIS API for JavaScript](https://developers.arcgis.com/javascript/) のブラウザベースのアプリケーションで 背景地図 や ルート検索サービス を使用する方法を示しています。
...
- [arcgis-rest-js-auth](./arcgis-rest-js-auth/README.md) は Vite を使用した [ArcGIS REST JS](https://developers.arcgis.com/arcgis-rest-js/) のブラウザベースのアプリケーションで OAuth 2.0 による [ArcGIS identity](https://developers.arcgis.com/documentation/mapping-apis-and-services/security/arcgis-identity/) を利用したユーザーのログイン方法を示しています。
...
- [esm-user-login-demo](./esm-user-login-demo/README.md) は ES モジュールを使用した [ArcGIS API for JavaScript](https://developers.arcgis.com/javascript/) のブラウザベースのアプリケーションで OAuth 2.0 による [ArcGIS identity](https://developers.arcgis.com/documentation/mapping-apis-and-services/security/arcgis-identity/) を利用してユーザーがログインし、背景地図 や ルート検索サービス を使用する方法を示しています。
...
- [nodejs-app-credential-example](./nodejs-app-credential-example/README.md) は [ArcGIS application credentials](https://developers.arcgis.com/documentation/mapping-apis-and-services/security/application-credentials/) で公開されているアプリ認証 の Node.js でのサンプルコードの実装です。
...
- [esm-app-token-demo](./esm-app-token-demo/README.md) は [ArcGIS application credentials](https://developers.arcgis.com/documentation/mapping-apis-and-services/security/application-credentials/) とバックエンドサーバーおよび OAuth 2.0 を使用してアプリケーション トークンを取得し、ES モジュール を使用した [ArcGIS API for JavaScript](https://developers.arcgis.com/javascript/) のブラウザベースのアプリケーションで、背景地図 や ルート検索サービス を使用する方法を示しています。このプロジェクトでは、トークンのネゴシエーションを処理するためのサーバーが必要であり、ここでは次の2つのオプションを用意しています:
...
<!--     - [nodejs-app-token-server-demo](./nodejs-app-token-server-demo/README.md) is a server written with Node.js.
    ...
    - [nodejs-app-token-server-demo](./nodejs-app-token-server-demo/README.md) は Node.js で書かれたサーバーです
    - [php-app-token-server-demo](./php-app-token-server-demo/README.md) は PHP で書かれたサーバーです
